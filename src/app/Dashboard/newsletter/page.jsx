...
const Newsletter = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/email-subscriptions/find?page=${currentPage}&limit=10&search=${searchTerm}`,
          {
            headers: {
    ...
    const handleDelete = async (id) => {
      try {
        await axios.delete(
          `${process.env.NEXT_PUBLIC_API_URL}/email-subscriptions/delete/${id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
