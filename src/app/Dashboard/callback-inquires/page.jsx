...
const RequestCallback = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/request-a-callback/find?page=${currentPage}&limit=10&search=${searchTerm}`,
          {
            headers: {
    ...
    const handleDelete = async (id) => {
      try {
        await axios.delete(
          `${process.env.NEXT_PUBLIC_API_URL}/request-a-callback/delete/${id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
